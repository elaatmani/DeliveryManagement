  ...
  <div>
    ...
    <div v-if="isLoaded" class="py-5 px-5 tw-border bg-white tw-w-full tw-rounded-md">
      <div class="tw-flex tw-gap-2 tw-text-neutral-800 tw-items-center tw-mb-5">
            Product ID: <span class="tw-block tw-py-1 tw-px-2 tw-rounded-md text-primary-color">{{ id }}</span>
        </div>
      <div>
      ...
      .then(
        res => {
          if (res.data.code == 'SUCCESS') {
            const {variations, buying_price, selling_price, ref, name, description} = res.data.data.products
