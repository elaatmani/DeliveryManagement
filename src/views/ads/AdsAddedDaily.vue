<template>
     <div class="py-5 px-2 tw-border tw-bg-white tw-w-full tw-rounded-md tw-mb-5 ">
      <h1 class="tw-text-xl tw-m-2 tw-pb-5 tw-font-medium tw-text-gray-800 darkx:tw-text-white">Appouving Leads</h1>
                      ...
                      <table class="tw-min-w-full tw-divide-y tw-divide-gray-200 darkx:tw-divide-gray-700">
                          <thead class="tw-bg-gray-50 darkx:tw-bg-gray-800">
                              <tr>
                                  ...
                                  <th scope="col" class="tw-py-3.5 tw-px-4 tw-text-sm tw-font-normal tw-text-left rtl:tw-text-right tw-text-gray-500 darkx:tw-text-gray-400">
                                    <button @click="$emit('sortOrder', {name: 'ads_at'})" class="tw-flex tw-items-center tw-gap-x-3 focus:tw-outline-none">
                                        <span>Ads For</span>
                          ...
                          <tbody v-if="!loading && adsAddedDaily.length > 0" class="tw-bg-white tw-divide-y tw-divide-gray-200 darkx:tw-divide-gray-700 darkx:tw-bg-gray-900">
                            <TableRowAddedAds v-for="ad in adsAddedDaily" :key="ad.id" :item="ad" @adApproved="removeAd" />
                          </tbody>
                          <tbody v-if="!loading && adsAddedDaily.length == 0" class="tw-bg-white tw-divide-y tw-divide-gray-200 darkx:tw-divide-gray-700 darkx:tw-bg-gray-900">
                            <tr>
                              <td colspan="9">
  ...
  import TableRowAddedAds from '@/views/ads/partials/table/TableRowAddedAds.vue'
  import Ads from '@/api/Ads';
  
  export default {
    components: { TablePaginationNoNums,TableRowAddedAds },
    data() {
      return {
        adsAddedDaily: [],
        from: 1,
    ...
    methods: {
      fetchAdsAddedToday() {
        return Ads.addedToday()
          .then(({ data }) => {
            this.adsAddedDaily = data.data.ads;
            this.total = data.data.total;
            this.currentPage = data.data.current_page;
            this.lastPage = data.data.last_page;
            this.from = this.currentPage * this.perPage - this.perPage + 1;
            this.to = Math.min(this.currentPage * this.perPage, this.total);
            this.loading = false;
          });
      },
      handlePageChange(page) {
        this.currentPage = page;
        this.fetchAdsAddedToday();
      },
      removeAd(adId) {
            console.log('removeAd called with id:', adId);
            this.adsAddedDaily = this.adsAddedDaily.filter(ad => ad.id !== adId);
        },
    },
    mounted() {
      this.fetchAdsAddedToday();
