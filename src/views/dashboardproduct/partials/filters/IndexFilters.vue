<template>
  <div>
    <div class="tw-mt-6 tw-mb-4 md:tw-flex md:tw-items-center md:tw-justify-between">
      <div
        ...
        <button :class="[showAll && '!tw-bg-gray-100']" :disabled="showAll" @click="filterToday(false)"
        ...
        <button title="Show Filters Tab" @click="showFilters = !showFilters" :class="[showFilters && '!tw-bg-gray-100']"
        ...
        <button title="Refresh" @click="$emit('fresh')" :disabled="loading"
          class="tw-px-5 tw-py-2 tw-text-xs tw-font-medium tw-border-solid tw-text-gray-600 tw-transition-colors tw-duration-200 sm:tw-text-sm darkx:hover:tw-bg-gray-800 darkx:tw-text-gray-300 hover:tw-bg-gray-100">
        ...
        <div class="tw-relative">
          <select @change="handlePerPageChange" :value="perPage"
    ...
    <div :class="[showFilters ? 'tw-grid-rows-[1fr]' : 'tw-grid-rows-[0fr]']"
      class="tw-grid tw-duration-300 tw-transition-all">

      <div class="tw-overflow-hidden tw-col-span-1">
        <FiltersWrapper @clear="$emit('clear')" @filter="$emit('filter')" :filters="filters"
          @update="v => $emit('update:filters', v)" />
      </div>
...
import FiltersWrapper from '@/views/dashboardproduct/partials/filters/FiltersWrapper'
