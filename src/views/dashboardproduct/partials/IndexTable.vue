<template>
  <div>
          ...
          <div class="tw-border tw-overflow-auto tw-border-gray-200 darkx:tw-border-gray-700 md:tw-rounded-lg">
            <table class="tw-min-w-full tw-divide-y tw-divide-gray-200 darkx:tw-divide-gray-700">
              <thead class="tw-bg-gray-50 darkx:tw-bg-gray-800">
                <tr>
                  <th scope="col"
                    class="tw-py-3.5 tw-px-4 tw-w-5 tw-text-sm tw-font-normal tw-text-left rtl:tw-text-right tw-text-gray-500 darkx:tw-text-gray-400">
                    <span>ID</span>
                  ...
                  <th scope="col"
                    class="tw-py-3.5 tw-px-4 tw-text-sm tw-font-normal tw-text-left rtl:tw-text-right tw-text-gray-500 darkx:tw-text-gray-400">
                    <button @click="$emit('sortOrder', { name: 'total_orders' })"
                      class="tw-flex tw-items-center tw-gap-x-3 focus:tw-outline-none">
                      <span>Ad Spend</span>
                  ...
                  <th scope="col"
                    class="tw-py-3.5 tw-px-4 tw-text-sm tw-font-normal tw-text-left rtl:tw-text-right tw-text-gray-500 darkx:tw-text-gray-400">
                    <button @click="$emit('sortOrder', { name: 'total_quantity' })"
                      class="tw-flex tw-items-center tw-gap-x-3 focus:tw-outline-none">
              ...
              <tbody v-if="!loading && items.length > 0"
                ...
                <TableRow @update="newItem => $emit('update', newItem)" :all="all" :index="index"
                  v-for="(item, index, all) in items" :key="index" :item="item" />
              </tbody>
              ...
              <tbody v-if="loading"
                class="tw-bg-white tw-divide-y tw-divide-gray-200 darkx:tw-divide-gray-700 darkx:tw-bg-gray-900">
                <tr>
                  <td v-if="false" colspan="11" class="tw-px-4 tw-py-4 tw-text-sm tw-font-medium tw-whitespace-nowrap">
                    <loading />
                  </td>
                  <td colspan="11">
                    ...
                    <div role="status"
                      ...
                      <div v-for="i in (perPage - 1)" :key="i"
    ...
    <div class="tw-mt-6">
      <TablePaginationNoNums :from="from" :to="to" :last-page="lastPage" :per-page="perPage" :total="total"
        :current-page="currentPage" @page-change="(n) => $emit('pageChange', n)" />
    </div>
  </div>
</template>

<script>
import TablePaginationNoNums from '@/views/dashboardproduct/partials/table/TablePaginationNoNums';
import TableRow from '@/views/dashboardproduct/partials/table/TableRow';
