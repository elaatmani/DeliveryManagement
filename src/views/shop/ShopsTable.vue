  ...
  <div>
    ...
    <div class="tw-relative tw-overflow-x-auto  sm:tw-rounded-lg">
        <table class="tw-w-full tw-text-sm tw-text-left tw-text-gray-500">
            ...
            <tbody>
                <tr v-for="shop in shops" :key="shop.id" class="tw-bg-white tw-border-b tw-whitespace-nowrap hover:tw-bg-gray-50">
        ...
        <div class="mt-5 tw-grid tw-grid-cols-12 tw-gap-5">
            ...
            <div class="tw-col-span-12 md:tw-col-span-6 d-flex tw-justify-end tw-flex-wrap align-center">
                <div class="text-caption tw-h-fit mr-2 font-weight-bold tw-text-zinc-700">{{ prevRange + 1 }} - {{ (currentPage == pageCount ?  shops.length : nextRange) }} of {{  shops.length }} items </div>
                <div>
...
export default {
    ...
    computed: {
        ...
        pageCount() {
            return Math.ceil(this.shops.length / this.paginationLimit)
        },
        items() {
            return this.shops.slice(this.prevRange, this.nextRange)
        }
    },
