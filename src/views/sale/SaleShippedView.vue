<template>
        ...
        <div class="tw-max-h-0 tw-duration-500 tw-overflow-hidden" :class="{'!tw-max-h-[500px]': showFilters}">
          <div class="tw-grid tw-grid-cols-12 tw-gap-2 tw-mb-4">
            ...
            <div class="lg:tw-col-span-4 md:tw-col-span-6 tw-col-span-12 tw-h-fit"> 
              ...
              <div  class="tw-relative">
                <select v-model="agenteFilter" class="tw-w-full focus:tw-border-orange-400 tw-h-[40px] px-2 tw-rounded-md tw-border tw-border-solid tw-border-neutral-200  tw-outline-0  tw-text-sm">
                  <option value="all">Select</option>
  ...
  export default {
    ...
    computed: {
      ...
      sales() {
        return this.$store.getters['sale/sales'].filter(sale => sale.delivery == 'expidier')
      },
      cities() {
