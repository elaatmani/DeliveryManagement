<template>
    <div class="tw-bg-white tw-p-4">
        <h2 class="tw-text-lg tw-font-semibold tw-mb-2">Income Distribution</h2>
        <!-- Skeleton Loader -->
        ...
        <div v-else>
            ...
            <div v-else class="tw-h-[320px] tw-flex tw-items-center tw-justify-center">
                <p class="tw-text-center tw-text-gray-700">No Income</p>
            </div>
        </div>
...
const getData = async () => {
    ...
    await CashFlowDashboard.incomesByCategory(filters.value)
        .then((res) => {
            if (res.data.code == 'SUCCESS') {
                chartOptions.value.labels = res.data.data.map(i => i.category_name);
                series.value = res.data.data.map(i => parseFloat(i.income));
