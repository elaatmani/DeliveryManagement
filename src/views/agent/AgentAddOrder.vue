  ...
  <div class="py-5 px-2 tw-border tw-bg-white tw-w-full tw-rounded-md tw-mb-5 ">
    ...
    <div v-if="!fetching && order_exists" class="tw-mt-5 tw-space-y-5">
        ...
        <template v-if="doubles.length" >
            <OrderCard v-for="db in doubles" :key="db.id" :order="db" @update="o => doubles = doubles.map(d => d.id == o.id ? o : d)" />
        </template>
...
<script>
import { toConfirm, getOrder, save } from '@/views/agent/partials/lib/add_order';
import OrderCard from '@/views/agent/partials/components/OrderCard';
