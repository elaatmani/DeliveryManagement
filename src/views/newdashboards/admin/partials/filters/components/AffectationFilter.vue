<template>
    <div class="md:tw-col-span-3 tw-col-span-12 tw-rounded">
        ...
        <select
          @change="e=> $emit('update', {...filters, affectation: e.target.value})"
        ...
        >
          ...
          <option :value="d.id" class="tw-capitalize" v-for="d in deliveries" :key="d.id">
...
export default {
    ...
    computed: {
        ...
        deliveries() {
            return this.fetched ? this.users.filter(u => u.role.name == 'delivery') : []
        },
        fetched() {
