  ...
  <div>
    <div v-if="!loading" class="tw-w-full tw-shadow-md tw-p-4 tw-bg-white tw-rounded-lg">
      <div class="tw-flex tw-flex-wrap tw-justify-between tw-items-center">
        <h1 class="tw-text-xl tw-font-medium tw-py-2">Total Handled or Treated :</h1>
...
const getData = async (filters) => {
  loading.value = true;
  try {
    const res = await Dashboard.treatedOrHandledOrders(filters);
    if (res.data.code === 'SUCCESS') {
      const rawData = res.data.data.ordersTreatedOrHandledByDate;
