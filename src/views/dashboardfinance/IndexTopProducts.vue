...
import IndexTable from "@/views/dashboardfinance/partials/IndexTable";
import IndexFilters from "@/views/dashboardfinance/partials/filters/IndexFilters";
...
export default {
  ...
  methods: {
    paginateProducts() {
      ...
      this.fetching = true;
      return Finance.get_top_products(url, options)
        .then(({ data }) => {
          const options = data.data.topProducts;
