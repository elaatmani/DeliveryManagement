<template>
    <div>
        <button @click="visible = true" class="tw-bg-whites tw-shadow-sm tw-px-2 tw-py-1 tw-w-[30px] tw-h-[30px] tw-border tw-border-solid tw-border-red-500/20 hover:tw-bg-red-600 tw-bg-red-500 hover:tw-border-red-500/70 tw-duration-300 tw-text-white tw-rounded-md tw-flex tw-items-center tw-justify-center">
...
const deleteItem = async () => {
    loading.value = true;
    await Blacklist.delete(props.item.id)
    .then(
        res => {
            if(res.data.code == 'SUCCESS') {
                blacklist.deleteItem(props.item.id);
